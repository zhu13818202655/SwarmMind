...
def test_envelope_success_typed() -> None:
    ...
    env = parse_envelope(payload, endpoint="/api/device/missions/getFlyStatis", data_model=FlyStatisResp)
...
def test_envelope_business_error_raises() -> None:
    with pytest.raises(DikongApiError, match="permission denied"):
        parse_envelope(
            {"code": 4001, "msg": "permission denied", "data": None},
            endpoint="/api/device/missions/getFlyStatis",
        )


def test_loose_typed_responses_keep_unknown_keys() -> None:
    raw = {"warnNum": 7, "categories": {"a": 1, "b": 2}}
    env = parse_envelope({"code": 0, "data": raw}, endpoint="/api/device/missions/getWarnStatic", data_model=WarnStaticResp)
    assert env.data is not None
    assert env.data.raw == raw

    env2 = parse_envelope({"code": 0, "data": {"alarms": 9}}, endpoint="/api/device/devices/hms/stats", data_model=HmsStatsResp)
...
def test_mission_query_by_page_aliases_list_field() -> None:
    ...
    env = parse_envelope(payload, endpoint="/api/device/missions/queryByPage", data_model=MissionQueryByPageResp)
    assert env.data is not None
