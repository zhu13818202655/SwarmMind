"""OpenSandbox-backed provider implementation."""
...
from swarmmind.sandbox.profiles import DEFAULT_PROFILES, SandboxProfile
...
class OpenSandboxAdapter(SandboxProvider):
    """Wrap OpenSandbox SDK behind the project-level provider interface."""
    ...
    def __init__(
        ...
        *,
        ...
        # Keep environment values for compatibility with SDK internals and external tools.
        os.environ["OPEN_SANDBOX_API_KEY"] = api_key
        os.environ["OPEN_SANDBOX_BASE_URL"] = base_url
        os.environ["OPEN_SANDBOX_DOMAIN"] = self._connection_config.get_domain()
    ...
    async def kill(self, sandbox_id: str) -> None:
        ...
        try:
            ...
            # Close local transport resources after remote kill.
            await sandbox.close()
    ...
    async def _create_sandbox(self, profile: SandboxProfile, metadata: dict[str, str]) -> Sandbox:
        """Create sandbox with cross-version compatibility for SDK keyword changes."""
        common_kwargs = {
        ...
        # Newer SDK examples often use `resourceLimits`, while 0.1.5 expects `resource`.
        try:
    ...
    def _build_connection_config(*, api_key: str, base_url: str) -> ConnectionConfig:
        """Convert base URL into SDK-native ConnectionConfig.

        opensandbox 0.1.x expects protocol + domain and appends /v1 internally.
        """
        ...
        # Accept users passing .../v1 and normalize to host[:port].
    ...
    def _extract_exit_code(execution: object) -> int:
        # Newer SDKs may expose exit_code directly.
        for attr in ("exit_code", "exitCode"):
            value = getattr(execution, attr, None)
            if isinstance(value, int):
                return value

        # opensandbox 0.1.x returns error.value as stringified code.
        error = getattr(execution, "error", None)
        if error is not None:
